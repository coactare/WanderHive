...
public class ProductRepository : IProductRepository, IBrandRepository, ITypesRepository
{
    ...
    public async Task<Pagination<Product>> GetProducts(CatalogSpecParams catalogSpecParams)
    {
        ...
        if (!string.IsNullOrEmpty(catalogSpecParams.Description))
        {
            if (catalogSpecParams.Description.ToLower() == "no-description")
            {
                var typeFilter = builder.Or(
                    builder.Eq(x => x.Description, null), 
                    builder.Eq(x => x.Description, "")
                );
                filter &= typeFilter;
            }
            else if (catalogSpecParams.Description.ToLower() == "with-description")
            {
                var typeFilter = builder.Ne(x => x.Description, null) & builder.Ne(x => x.Description, "");
                filter &= typeFilter;
            }
            else
            {
                var all = catalogSpecParams.Description.ToLower();
            }
        }
